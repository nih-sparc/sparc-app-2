...
const { data: dashboardData } = await useAsyncData('dashboardPage', async () => {
  const pageData = await $contentfulClient.getEntry(config.public.ctf_sparc_dashboard_entry_id)
  return {
    fields: pageData.fields || {},
  }
})

const title = computed(() => pathOr('', ['fields', 'name'], dashboardData.value))
const description = computed(() => pathOr('', ['fields', 'longDescription'], dashboardData.value))
</script>
