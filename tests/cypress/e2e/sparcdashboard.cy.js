...
describe('SPARC Dashboard', { testIsolation: false }, function () {
  ...
  it('Dashboard page loads correctly', function () {
    ...
    cy.get('.page-hero-content .el-button').should(($button) => {
      expect($button, 'User Guide button should exist').to.contain('View User Guide')
    })
  ...
  it('FlatmapWidget loads correctly', function () {
    ...
    cy.get('body').then(($body) => {
      if ($body.find('.flatmap-container').length > 0) {
        ...
        cy.get('[class*="flatmap"]').first().should(($flatmap) => {
  ...
  it('Breadcrumb navigation is correct', function () {
    // Check for breadcrumb navigation text
    cy.contains('Home', { timeout: 60000 }).should(($link) => {
      expect($link, 'Home breadcrumb should exist').to.exist
    })
    ...
    cy.contains('Human Vagus Nerve Explorer', { timeout: 60000 }).should(($text) => {
